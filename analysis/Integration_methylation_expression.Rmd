...
methyl_values <- read.csv("~/Desktop/Reg_Evo_Primates/data/3965_avg_methyl_per_ts_gene.txt", sep="", stringsAsFactors=FALSE)
...
median(run1000[1,1:750])
median(run1000[2,1:750])
median(run1000[3,1:750])
...
human_rhesus_liver_samples <- c(18, 22, 26, 30, 34, 38, 42, 46)
species_no_rhesus <- species[human_rhesus_liver_samples]
species_no_rhesus <- droplevels.factor(species_no_rhesus, "Chimp")

# Make the design matrix
design <- model.matrix(~ 0 + as.factor(species_no_rhesus) + RIN[human_rhesus_liver_samples,])
...
for (i in 1:3776){
    resid_methyl[i,] <- lm(t(expression_values_only[i,human_rhesus_liver_samples]) ~ t(methylation_values_only[i,human_rhesus_liver_samples]))$resid
}
...
fit1 <- lmFit(expression_values_only[,human_rhesus_liver_samples], design)
fit1 <- eBayes(fit1)
HvCLiver_fit1 = topTable(fit1, coef=2, adjust="BH", number=Inf, sort.by="none")
...
DE_total <- HvCLiver_DE_fits12[which(HvCLiver_DE_fits12$fit1_FDR < FDR_level &  HvCLiver_DE_fits12$fit2_FDR < FDR_level),]
nrow(DE_total)
...
for (i in 1:3776){
  df <- as.data.frame(methylation_values_only[i,human_rhesus_liver_samples])
  new_df <- as.data.frame(sample(df))
  run1[i,] <- t(new_df)
}
...
for (i in 1:3776){
    resid_methyl_perm[i,] <- lm(t(expression_values_only[i,human_rhesus_liver_samples]) ~ run1[i,])$resid
}
...
HvCLiver_DE_perm <- cbind(HvCLiver_DE_fits12$fit1_FDR, HvCLiver_perm$adj.P.Val)
