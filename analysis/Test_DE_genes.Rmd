...
library("limma")
library("VennDiagram")
source("~/Desktop/Reg_Evo_Primates/analysis/functions.R")
...
#GC.loess.norm.voom <- read.delim("~/Desktop/Reg_Evo_Primates/data/gc_cyclic_loess_random_var_gene_exp_counts")

samples <- read.delim("~/Desktop/Reg_Evo_Primates/data/Sample_info_RNAseq_limma.txt")
...
counts_genes_in_cutoff <- read.delim("~/Desktop/Reg_Evo_Primates/data/counts_12184.txt")
...
# Make 
dev.off()
Four_comp <- venn.diagram(mylist, filename= NULL, main="DE genes between Humans and Rhesus (FDR 1%)", cex=1.5 , fill = pal[1:3], lty=1, height=2000, width=3000)
grid.draw(Four_comp)
