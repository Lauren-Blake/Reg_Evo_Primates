...
end <- as.data.frame(coor_num_minus_one, stringsAsFactors = FALSE)
...
gene_20000_freq_table <- count(cpg_with_genes, c('gene'))
...
avg_methylation_multiple_cpgs_per_gene <- array(data = NA, dim = c(5255,48))
...
val_last_gene <- human.cpg.intersect_2cpgs[as.logical(ave(human_chimp_rhesus_2_orth_cpgs$start.b.x, human_chimp_rhesus_2_orth_cpgs$gene.x, FUN = function(x) x == max(x))),]
...
methyl_250_2cpgs <- read.csv("~/Desktop/Reg_Evo_Primates/data/methyl_250_2cpgs.txt", sep="", stringsAsFactors=FALSE)
...
Now we are going to run bedtools with the intersection of 
...
cors <- cor(avg_methylation_multiple_cpgs_per_gene, method="pearson", use="pairwise.all.obs")
